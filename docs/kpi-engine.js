class TeamKPIEngine {
                ...
                try {
                    ...
                    for (let i = headerIdx + 1; i < data.length; i++) {
                        ...
                        for (let j = 0; j < headers.length; j++) {
                            ...
                            // Several trackers repeat a header name later in the
                            // row (e.g. a second, mostly-blank "Status" column) -
                            // keep only the first occurrence so it can't silently
                            // overwrite the real value with a blank.
                            if (seenHeaders.has(headers[j])) continue;
                            seenHeaders.add(headers[j]);
            ...
            getNHPending(name) {
                // Attributed to the Processor (who did the work), not the
                // Auditor (who reviewed it) - matching on Auditor was
                // wrongly attributing almost every pending case to whichever
                // Internal Audit teammate happened to review it, instead of
                // the person actually responsible for the pending item.
                if (!this.excelData.nhPending) return null;
                let found = false;
                let pendingCount = 0;
                this.excelData.nhPending.forEach(row => {
            ...
            getClientPending(name) {
                // Same logic as NH Pending: if Status = "Pending", count it
                // against that Team Member; if "Completed", it contributes 0.
                // Distinguish "never in this tracker" (no data) from "in it,
                // all completed" (a real 0) - this file only covers certain
                // teams, so an absent employee shouldn't show a false "0".
                if (!this.excelData.clientAudit) return null;
                let found = false;
                let pendingCount = 0;
                this.excelData.clientAudit.forEach(row => {
                    const member = row['Team Member'];
                    if (!member || !this.nameMatches(member, name)) return;
                    found = true;
                    const status = (row['Status'] || '').toString().trim().toLowerCase();
                    if (status === 'pending') pendingCount++;
                });
